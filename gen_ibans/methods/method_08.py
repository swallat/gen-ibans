"""
Method 08: Modulus 11 with weights [2,3,4,5,6,7,8,9,2] applied right-to-left over 9 payload digits.

Rules (same family as methods 02/24):
...
def _compute_check_mod11_m08(payload: str) -> int | None:
...
def validate_method_08(blz: str, account: str) -> bool:
    ...
    expected = _compute_check_mod11_m08(payload)
    if expected is None:
        return False
...
def generate_account_method_08(blz: str, rng: __import__("random").Random) -> str:
    for _ in range(1000):
        ...
        cd = _compute_check_mod11_m08(payload)
        if cd is not None:
